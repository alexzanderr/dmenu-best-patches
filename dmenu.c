...
// andrew lib
#include "andrew_mod/from_andrew.h"
...
static void keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		// deletes everything from cursor to right
		...
		// added by alexzander
		// deletes everything from input box
		case XK_a: // sterge, dar dupa nu mai pot scriu, acum merge de la match
			text[0] = '\0';
			cursor = 0;
			match();
			break;
		...
		// added by alexzander
		// control delete
		case XK_Delete: // deletes entire word to right
			if (text[cursor] == '\0')
				return;

			// paste your modifcations here
		...
		// added by alexzander
		// deletes everyting from cursor to left
		case XK_w: /* delete left */
			insert(NULL, 0 - cursor);
			break;
		...
		// added by alexzander
		// deletes and entire word to left
		// i want to do that with control+backspace
		case XK_BackSpace:
			// sterge secvential, dar nu-mi sterge primul cuvant scris
			while (cursor > 0 && strchr(worddelimiters, text[nextrune(-1)]))
				insert(NULL, nextrune(-1) - cursor);
			while (cursor > 0 && !strchr(worddelimiters, text[nextrune(-1)]))
				insert(NULL, nextrune(-1) - cursor);
...
insert:
    ...
    case XK_Return:
	case XK_KP_Enter:
        // text - is the text you inserted from keyboard
        // sel->text - is the text autocompletion for what you inserted from keyboard

        // aici apesi enter ca sa rulezi ce ai scris


        // merge
        // system(string_formatting("notify-send title %s", text));
        // char * result = compute_math_expression(text);
        // sprintf(t, "notify-send %d", is_math_exression(text));
        // system(t);

        if (is_math_exression(text)) {
            char * result = compute_math_expression(text);
            system(string_formatting_2("notify-send '%s' '==   %s'", text, result));
        }
        else if (is_valid_url(sel->text))
            open_in_default_browser(sel->text);
        // else if (is_valid_url(text))
